...
def export_report():
    ...
    md_content = "# 📊 Lehman Week Report (Commercial v2.0)\n\n"
    ...
    for r in daily_reports:
        try:
            ...
            md_content += f"- Secondary: {drivers.get('secondary', 'N/A')}\n"
            ...
            # Trader Playbook (NEW)
            playbook = content.get('trader_playbook', {})
            md_content += f"**♟️ Trader Playbook**:\n"
            md_content += f"- Short-Term: {playbook.get('short_term_scenario', 'N/A')}\n"
            md_content += f"- Mid-Term: {playbook.get('mid_term_outlook', 'N/A')}\n\n"
            
            # Watchlist (NEW)
            watchlist = content.get('tomorrows_watchlist', [])
            md_content += f"**👀 Watchlist**: {', '.join(watchlist)}\n\n"
    ...
    with open("lehman_week_report_commercial_v2.md", "w") as f:
        f.write(md_content)
    
    print("Exported to lehman_week_report_commercial_v2.md")
