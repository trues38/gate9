...
def enrich_game(game_id, home_id, away_id):
    ...
    print(f"   📡 Market Data: {summary_data.get('odds')}")
    
    # 3. Injuries (From Roster Endpoints)
    print(f"   🚑 Fetching Roster Health (Home: {home_id}, Away: {away_id})...")
    home_injuries = espn.fetch_roster(home_id)
    away_injuries = espn.fetch_roster(away_id)
    
    # 4. Combine
    enriched = {
        "game_id": game_id,
        "referee_context": ref_data,
        "market_context": summary_data.get('odds'),
        "injury_context": {
            "home": home_injuries,
            "away": away_injuries
        }
