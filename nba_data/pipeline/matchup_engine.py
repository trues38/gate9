...
class MatchupEngine:
    ...
    def _select_key_players(self, team_id, roster_names=None, top_n=4):
        # Need to map player name -> team_id -> regime
        # current_regimes.json is a LIST of players
        
        candidates = []
        
        # 1. Use Live Roster (Best)
        if roster_names and isinstance(self.player_regimes, list):
             for p in self.player_regimes:
                 # Check if player name exists in live roster
                 if p.get('name') in roster_names:
                     candidates.append(p)
                     
        # 2. Fallback to Stale Schema (if roster fetch failed)
        elif isinstance(self.player_regimes, list):
            for p in self.player_regimes:
    ...
    def build_context(self, game_id: str) -> dict:
        ...
        # 2. Extract Team IDs (Dynamic)
        # enriched data now contains team metadata in 'teams' keys or requires inference
        # For robustness, we assume 23_data_enrichment passed basic metadata or we parse from matchup
        # Since 23_data_enrichment structure is loose, we rely on 'home_id' and 'away_id' being passed or inferred
        # FIX: We will read from 'meta' if available, otherwise fail gracefully
        
        home_id = enriched.get('home_id')
        away_id = enriched.get('away_id')
        
        if not home_id or not away_id:
            # Emergency: Try to parse from filename or fallback if strictly necessary
            # For now, let's look for keys in 'teams' dict if it exists
            pass 
        ...
        crew_chief = ref_context.get('crew_chief', 'Unknown')
        ref_db_data = self.ref_regimes.get(crew_chief, self.ref_regimes.get('Generic'))

        # 4. Key Players
        rosters = enriched.get('rosters', {})
        home_roster_names = rosters.get('home', [])
        away_roster_names = rosters.get('away', [])
        
        home_players = self._select_key_players(home_id, roster_names=home_roster_names)
        away_players = self._select_key_players(away_id, roster_names=away_roster_names)
        
        # 5. Team Metadata (Name resolution)
        # Try to find team name from regime data or roster map
        home_name = home_regime.get('team_name', f"Team {home_id}")
        away_name = away_regime.get('team_name', f"Team {away_id}")
