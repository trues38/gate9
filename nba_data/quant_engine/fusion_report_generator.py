...
def generate_markdown_report(date_str, matches):
    ...
    for m in matches:
        ...
        if m.get('twin_data'):
            ...
            md_content += f"> Narrative Sim: {t.get('narrative_similarity',0)*100:.1f}%\n"
            md_content += f"> Context Sim: {t.get('context_similarity',0)*100:.1f}%\n"
            md_content += f"> **Fusion Score: {t.get('final_score',0)*100:.1f}**\n"
        elif twin != 'Active':
             md_content += "> *Twin Engine Skipped (Game Type Safe/Low Risk)*\n"
        else:
            md_content += "> *No significant historical twin detected (Despite Risk Alert).*\n"
...
def main():
    ...
    # Run Fusion Engine (Cache Based V1.0)
    import sys
    ...
    # --- PHASE 2: TWIN ENGINE (Gated) ---
    print(f">>> Phase 2: Qualitative Twin Engine Scan (Gated)")
    twin_engine = find_twin_upset_v2.TwinEngineV2()
    
    for match in matches:
        # GATING LOGIC
        if match.get('twin_alert', 'None') != 'Active':
            print(f"Skipping Twin Engine for {match['game_id']} (Type: {match.get('game_type','Unknown')})")
            continue

        print(f"Running Twin Engine for {match['game_id']} (Risk Triggered)...")

        # Build Context for Twin Search
        context = {
            "location": "HOME",
            "streak": 0, # Placeholder
            "rest_days": match.get('home_rest', 1)
        }
