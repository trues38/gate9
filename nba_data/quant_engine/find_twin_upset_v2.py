...
class TwinEngineV2:
    ...
    def find_twins(self, current_game_context, narrative_vector):
        ...
        for game in self.library:
            ...
            # Bonus 2: Location
            if current_game_context.get('location') == game_ctx.get('location'):
                ctx_score += 0.2
            ...
            scored_candidates.append({
                ...
                "context_match": f"Rest:{hist_rest}, Loc:{game_ctx.get('location')}"
            })
            
        # Sort and Return Top 3 (Strict Threshold)
        scored_candidates.sort(key=lambda x: x['final_score'], reverse=True)
        
        # Threshold: Only return if score >= 0.70 (Strict)
        # Previous user complaint: "60% vs 40% upset" logic.
        # Actually, let's filter by threshold.
        
        filtered = [x for x in scored_candidates if x['final_score'] >= 0.70]
