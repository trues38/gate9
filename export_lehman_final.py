...
def export_report():
    ...
    md_content = "# 📊 Lehman Week Report (Final Perfect Version)\n\n"
    ...
    for r in daily_reports:
        try:
            ...
            # Z-Score Analysis
            zscore = content.get('zscore_focus', {}) # New Key
            if not zscore: zscore = content.get('zscore_analysis', {}) # Fallback
            
            md_content += f"**📉 Z-Score Focus**: {zscore.get('top_sector', 'N/A')}\n"
            md_content += f"- **Chain**: {' -> '.join(zscore.get('event_chain', []))}\n\n"
    ...
    with open("lehman_week_report_final.md", "w") as f:
        f.write(md_content)
    
    print("Exported to lehman_week_report_final.md")
